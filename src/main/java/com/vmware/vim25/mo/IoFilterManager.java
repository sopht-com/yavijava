...
public class IoFilterManager extends ManagedObject {
     ...
     * compute resource.
     * @throws AlreadyExists   - Thrown if another VIB with the same name and vendor has been installed.
     * @throws InvalidArgument - Thrown if "compRes" is a standalone host.
     * @throws RuntimeFault    - Thrown if any type of runtime fault is thrown that is not covered by the other faults; for example,
     ...
     * privilege check ensures that the user must have Host.Config.Patch privilege for all the hosts in the
     * compute resource.
     * @throws FilterInUse     Thrown if the filter to be uninstalled is being used by a virtual disk.
     * @throws InvalidState    Thrown if "compRes" is a cluster and DRS is disabled on the cluster.
     * @throws NotFound        Thrown if the filter is not installed on the cluster. - Thrown if another VIB with the same name and
